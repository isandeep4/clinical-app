...
function CollectClinicals(){
    ...
    useEffect(()=>{
        axios.get("http://localhost:8080/clinicalservices/api/patients/"+patientId).then(res=>setPatientData(res.data));
        setLoading(false);
    },[]);
 
    const handleSubmit=(event)=>{
        const data = {patientId, componentName, componentValue};
        axios.post("http://localhost:8080/clinicalservices/api/clinicals", data).then(res=>console.log(res.data));
